...
上のようなエラーメッセージが表示される場合は，UEFIの設定画面を開き（スタートボタンを右クリック→設定→更新とセキュリティ→回復→今すぐ再起動する→トラブルシューティング→UEFIファームウェアの設定→再起動），「Virtual Technology（Intelの場合）」や「Virtualization Technology（Intelの場合）」，「SVM（AMDの場合）」などを有効にする。UEFIの設定画面はPCの起動時に開くこともできるが，その方法は機種によって異なる（起動時にdelete・F1・Alt+F4などのキーを押し続ける）。
