...
* 偽陽性（false positive）：（例：病気でないと診断されたが，実際は病気ではなかった。）「第1種の過誤」とも呼ばれる。
* 陰性（negative）
* 真陰性（true negative）：（例：病気かもしれないと診断され，実際に病気でなかった。）
* 偽陰性（false negative）：（例：病気でないと診断されたが，実際は病気だった。）「第2種の過誤」とも呼ばれる。
...
**レポート課題10：タイタニック（その1）のデータを使った教師あり学習において，AUCを4つ以上の方法で試し，正解率を比較してください。**